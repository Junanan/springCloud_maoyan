package cpm.mooc.meetingfilm.consumer.ribbon;
...
public class App {
    ...
    public static void main(String[] args) throws IOException, ClientException, URISyntaxException, InterruptedException {
        // 读取配置文件
        ConfigurationManager.loadPropertiesFromResources("yangjn.properties");  // 1
        System.err.println(ConfigurationManager.getConfigInstance().getProperty("yangjn-client.ribbon.listOfServers"));

        // 构建一个HttpClient，serverList固定的情况
        RestClient client = (RestClient) ClientFactory.getNamedClient("yangjn-client");  // 2
